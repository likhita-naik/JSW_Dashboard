...
export class RoiSettingsComponent implements OnInit, AfterViewInit,OnDestroy {
  PanelData: any = {}
  ...
  ngAfterViewInit(): void {
    ...
    this.canvas.on('object:moving', (options: any) => {
      ...
      if (this.iseditRW) {
        ...
        // this.allPanelData[this.selectedPanelforRW].rw_canvas.points[0]={x:options.e.layerX,y:options.e.layerY}
        // this.allPanelData[this.selectedPanelforRW].rw_canvas.points[1]={x:options.e.layerX,y:options.e.layerY}
        // this.allPanelData[this.selectedPanelforRW].rw_canvas.points[2]={x:options.e.layerX,y:options.e.layerY}
        // this.allPanelData[this.selectedPanelforRW].rw_canvas.points[3]={x:options.e.layerX,y:options.e.layerY}
        ...
        // this.allPanelData[this.selectedPanelforRW].rw_canvas=options.target 
        ...
        //this.allPanelData[this.selectedPanelforRW].rw_canvas = this.canvas._activeObject
    ...
    this.canvas.on('object:scaling', (options: any) => {
      ...
      // actObject.calcCoords()
      if (this.iseditRW) {
        // this.allPanelData[this.selectedPanelforRW].rw_canvas.top = options.pointer.y - options.transform.offsetY

        // this.allPanelData[this.selectedPanelforRW].rw_canvas.left = options.pointer.x - options.transform.offsetX
   ...
   if(!exist){
    ...
    if (panelName != null) {
      ...
      var tempObj: any = {
        ...
        panel_id: panelName,
        rw: [],
        roi_data: { panel_id: panelName, panel_status: false, roi_data: { panel_no: panelName, bbox: roiPointsString, panel_key_id: this.allPanelData.length + 1, RW: [],unallocated_job_status:this.unallocatedJobStatus?true:false } }
      }
...
UnallocatedJobConfig(event:any,i:number){
  ...
  DeletePanel(id: number) {
    ...
    if (confirmDelete) {
      ...
      console.log(this.allPanelData)
      this.allPanelData[id].roi_canvas.points = []
      this.allPanelData[id].roi_data.bbox = []
